...
void* counter_thread(void* arg)
{
    ...
    while (time(NULL) < end_time) {
	    ...
	    if (status != 0 ) err_abort(status, "Counter Lock Mutex thread");
        for (spin = 0; spin < SPIN; spin++) counter++;
        status = pthread_mutex_unlock(&mutex);
        if (status != 0 ) err_abort(status, "Counter Unlock Mutex thread");
        sleep(1);
    }
...
void* monitor_thread(void* arg)
{
    ...
    while (time(NULL) < end_time) {
        ...
        if (status != EBUSY) {
            if (status != 0) err_abort(status, "Monitor Trylock mutex");
            printf("Counter is %ld\n",counter/SPIN);
            status = pthread_mutex_unlock(&mutex);
            if (status != 0) err_abort(status, "Monitor Unlock mutex");
        } else
            misses++;
    }
...
int trylockMain(int argc, const char * argv[])
{
    int status;
